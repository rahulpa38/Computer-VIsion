...
# 	'''
    ...
    width = im.shape[0]*scale_factor
    height = im.shape[1]*scale_factor
    resize_image = np.zeros(shape=(width,height,3))
    for W in range(width):
        for H in range(height):
###  Map new width to old width using scale factor            
            new_width = int(W * (1/scale_factor))
            new_height = int(H * (1/scale_factor))
            resize_image[W][H] = im[new_width][new_height]
